...
// For vertex data
use cgmath::{Deg, SquareMatrix, point3, vec2, vec3};
use std::mem::size_of;
...
impl App {
    ...
    unsafe fn render(&mut self, window: &Window) -> Result<()> {
        ...
        // 1. Ждём завершения предыдущего использования этого frame
        self.device
...
unsafe fn create_swapchain_image_views(device: &Device, data: &mut AppData) -> Result<()> {
    data.swapchain_image_views = data
        .swapchain_images
        .iter()
        .map(|i| create_image_view(device, *i, data.swapchain_format, 1))
...
unsafe fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    let vert = include_bytes!("../shaders/spirv/vert.spv");
    let frag = include_bytes!("../shaders/spirv/frag.spv");
...
unsafe fn create_buffer(
    ...
    // It should be noted that in a real world application, you're not supposed to actually call allocate_memory for every individual buffer.
